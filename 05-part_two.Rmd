...
As this is a chapter about testing code, we going to focus primarily on unit testing. The other testing aspects cover testing of wider areas of a software project than just the code, and other parts of the DfE QA framework cover how to handle these:
...
To implement unit tests, it is generally easiest to use a unit testing framework. These generally allow tests to be writen separate files and then all tests run at once, returning success/failure feedback.
...
In `testthat`, tests are called **expectations**. These spell out the expected behaviour of the function or unit of code. `testthat` provides many functions to define expectations - see details [here](http://r-pkgs.had.co.nz/tests.html). It is good practice to save tests relating to the different code files in different test files, all saved within a dedicated testing directory. Below is an example of testing a simple function.
...
R Shiny apps may benefit from a further extended testing approach. They encompass not just some analysis, but also a User Interface and a Server to enable them to run. This makes them closer to a software development project than the average piece of analysis work.
...
"Test driven development" describes a way of working in software where tests are written prior to the code being written. This forces developers to sit down and work out what working code encompasses before the write it and avoids any retrofitting of tests to match achieved output. With a strong QA plan, this could also be implement for analysis. As testing of each new bit of code happens immediately after creation it also reduced the risk of something faulty getting heavily baked in.

As with testing, there are plenty of resources on the internet such as [this one](https://www.guru99.com/test-driven-development.html).