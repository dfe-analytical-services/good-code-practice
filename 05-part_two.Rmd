...
## Console tests
Testing code within the R (or Python) console is not recommended as it leaves no trace or records of what has been checked.
...
In Python similar approaches can be used. There is also a [print function](https://www.w3schools.com/python/ref_func_print.asp) and exceptions provide a way returning an error if a specified condition is met, much like the functionality offered by R's `stop()` and `stopifnot()` functions. [Here](https://www.w3schools.com/python/gloss_python_raise.asp) is information about how to raise exceptions and [here](https://docs.python.org/3/library/exceptions.html) is more technical, detailed information about the different types of exceptions.
...
R Shiny apps may benefit from a further extended testing approach. They encompass not just some analysis, but also a User Interface and a Server to enable them to run. This makes them closer to a software development project than the average piece of analytical work. Additional functionality is available within R to support testing of Shiny apps. Further details about this can be found [here](https://shiny.rstudio.com/articles/integration-testing.html).
